...
export async function PATCH(
  ...
  try {
    ...
    if (IS_DEV) {
      console.log(`[TICKETS-PROXY] PATCH close ticket: ${id}`);
    }

    const res = await fetch(`${API_BASE}/api/v1/portal/tickets/${encodeURIComponent(id)}/close`, {
      method: "PATCH",
      headers: {
    ...
    if (IS_DEV) console.error("[TICKETS-PROXY] PATCH Close error:", error);
